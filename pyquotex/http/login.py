...
class Login(Browser):
    ...
    def get_profile(self):
        ...
        if self.response:
            script_content = "{}"
            scripts = self.get_soup().find_all("script", {"type": "text/javascript"})
            for s in scripts:
                if "window.settings" in s.get_text():
            ...
            for i, s in enumerate(scripts):
                if "window.settings" in s.get_text():
                    txt = s.get_text()
                    try:
                        extracted = re.sub("window.settings = ", "", txt.strip().replace(";", ""))
    ...
    def success_login(self):
        # Fix: Check for /trade path specifically, as domain market-qx.trade contains "trade"
        if "/trade" in self.response.url and "/sign-in" not in self.response.url:
        ...
        if not match:
            # Try finding other error messages common on this site
            text_content = html.text
            if "E-mail ou senha incorretos" in text_content or "Incorrect email or password" in text_content:
                return False, "Login failed: Incorrect email or password."
            if "Too many attempts" in text_content:
                return False, "Login failed: Too many attempts."
        
        message_in_match = match.text.strip() if match else ""
        return False, f"Login failed. {message_in_match or 'Unknown error (stuck on login page)'}"
        if not match:
            # Try finding other error messages common on this site
            if "E-mail ou senha incorretos" in html.text or "Incorrect email or password" in html.text:
                return False, "Login failed: Incorrect email or password."
